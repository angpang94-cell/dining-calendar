...
let filter = { start: iso(startOfMonth(today)), end: iso(endOfMonth(today)), label: '이번 달', categories: [...categories, uncategorized] };
let editingId = null;
...
function rangeSummary(rs) { const total = sum(rs), diningCount = rs.filter(r => r.category !== '대리비').length, category = mostSpentCategory(rs); return `<div class="range-summary"><span>조회 기간</span><strong>${filter.start.replaceAll('-', '.')} ~ ${filter.end.replaceAll('-', '.')}</strong><div class="summary-grid"><div><span>총 외식비</span><b>${money(total)}</b></div><div><span>외식 횟수</span><b>${diningCount}회</b></div><div><span>1회 평균</span><b>${money(rs.length ? total / rs.length : 0)}</b></div><div><span>가장 많이 사용</span><b>${category || '-'}</b></div></div></div>`; }
...
function categoryName(category) { return category === uncategorized ? '미설정' : category; }
function categoryFilterPanel() { return `<section class="category-filter"><div class="section-heading"><div><h2>항목 선택</h2><p>여러 항목을 선택하면 합산해서 보여드려요.</p></div></div><div class="category-chips">${[...categories, uncategorized].map(category => `<button type="button" class="chip ${filter.categories.includes(category) ? 'selected' : ''}" data-category-filter="${category}" aria-pressed="${filter.categories.includes(category)}">${categoryName(category)}</button>`).join('')}</div></section>`; }
function categoryTotals(rs) { const totals = [...categories, uncategorized].map(category => [category, sum(rs.filter(record => (record.category || uncategorized) === category))]).filter(([, total]) => total > 0); return `<section class="category-totals section-block"><div class="section-heading"><h2>항목별 총합</h2></div>${totals.length ? `<div class="category-total-list">${totals.map(([category, total]) => `<div class="category-total-row"><span>${categoryName(category)}</span><strong>${money(total)}</strong></div>`).join('')}</div>` : `<div class="empty-state"><p>선택한 항목에 기록이 없어요.</p></div>`}</section>`; }
...
function savingsChart(entries) { const points = [...entries].sort((a, b) => a.date.localeCompare(b.date) || a.updatedAt - b.updatedAt).slice(-8); const values = points.map(point => point.amount); const min = Math.min(...values); const max = Math.max(...values); const range = Math.max(max - min, 1); const width = 360; const height = 190; const pad = { left: 14, right: 14, top: 20, bottom: 36 }; const plotWidth = width - pad.left - pad.right; const plotHeight = height - pad.top - pad.bottom; const x = index => pad.left + (points.length === 1 ? plotWidth / 2 : index * plotWidth / (points.length - 1)); const y = value => pad.top + (max - value) / range * plotHeight; const line = points.map((point, index) => `${x(index).toFixed(1)},${y(point.amount).toFixed(1)}`).join(' '); const area = `${pad.left},${height - pad.bottom} ${line} ${width - pad.right},${height - pad.bottom}`; const labelIndexes = [...new Set([0, Math.floor((points.length - 1) / 2), points.length - 1])]; const compactMoney = value => value >= 100000000 ? `${(value / 100000000).toFixed(1).replace('.0', '')}억` : value >= 10000 ? `${Math.round(value / 10000).toLocaleString('ko-KR')}만` : `${Math.round(value).toLocaleString('ko-KR')}`; return `<section class="savings-chart chart-section"><div class="section-heading"><div><h2>저축액 흐름</h2><p>최근 ${points.length}개 기록 기준</p></div><span class="chart-range">${compactMoney(min)} ~ ${compactMoney(max)}</span></div><div class="savings-chart-frame"><svg viewBox="0 0 ${width} ${height}" role="img" aria-label="저축액 변화 그래프"><line class="chart-grid-line" x1="${pad.left}" y1="${pad.top}" x2="${width - pad.right}" y2="${pad.top}"/><line class="chart-grid-line" x1="${pad.left}" y1="${height - pad.bottom}" x2="${width - pad.right}" y2="${height - pad.bottom}"/><text class="chart-y-label" x="${pad.left}" y="${pad.top - 6}">${compactMoney(max)}</text><text class="chart-y-label" x="${pad.left}" y="${height - pad.bottom + 17}">${compactMoney(min)}</text><polygon class="chart-area" points="${area}"/><polyline class="chart-line" points="${line}"/>${points.map((point, index) => `<circle class="chart-point" cx="${x(index).toFixed(1)}" cy="${y(point.amount).toFixed(1)}" r="4"><title>${point.date.replaceAll('-', '.')} · ${money(point.amount)}</title></circle>`).join('')}${labelIndexes.map(index => `<text class="chart-x-label" x="${x(index).toFixed(1)}" y="${height - 12}" text-anchor="${index === 0 ? 'start' : index === points.length - 1 ? 'end' : 'middle'}">${points[index].date.slice(5).replace('-', '.')}</text>`).join('')}</svg></div></section>`; }
...
function assetsView() { const entries = [...state.savings].sort((a, b) => b.date.localeCompare(a.date) || b.updatedAt - a.updatedAt); const change = savingsChange(); return `${topbar('자산', `<button class="icon-button" data-action="add-savings" aria-label="저축액 기록 추가">${icon('plus')}</button>`, '우리 가족의 저축 흐름을 함께 살펴봐요') }<div class="page-content assets-page">${savingsCharacter()}${change ? `<section class="asset-change"><span>최근 변화</span><strong class="${change.amount >= 0 ? 'up' : 'down'}">${change.amount > 0 ? '+' : ''}${money(change.amount)}</strong><small>${change.latest.date.replaceAll('-', '.')} 기준 · 직전 기록과 비교</small></section>` : ''}${entries.length >= 2 ? savingsChart(entries) : ''}<section class="section-block"><div class="section-heading"><h2>저축액 변화 기록 <span>${entries.length}</span></h2></div>${entries.length ? `<div class="savings-list">${entries.map((entry, index) => savingsEntryRow(entry, index, entries)).join('')}</div>` : `<div class="empty-state"><p>아직 저축액 기록이 없어요.</p><button class="secondary-button" data-action="add-savings">현재 저축액 입력</button></div>`}</section><section class="asset-tip"><strong>기록하는 방법</strong><p>잔액이 바뀔 때마다 현재 금액을 남기면 가족의 저축 흐름이 이어져요.</p></section></div>${nav()}`; }
function recordsView() { const rs = recordsBetween(filter.start, filter.end).filter(record => filter.categories.includes(record.category || uncategorized)); return `${topbar('기록', `<button class="icon-button" data-action="add" aria-label="기록 추가">${icon('plus')}</button>`)}<div class="page-content"><div class="page-intro"><p>내가 먹은 맛있는 순간을 모아봤어요.</p></div>${filterPanel()}${categoryFilterPanel()}${rangeSummary(rs)}${categoryTotals(rs)}<section class="section-block"><div class="section-heading"><h2>외식 내역 <span>${rs.length}</span></h2></div>${rs.length ? `<div class="record-list">${rs.map(recordRow).join('')}</div>` : emptyState('선택한 기간과 항목에 외식 기록이 없어요.')}</section></div>${nav()}`; }
