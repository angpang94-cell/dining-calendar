...
async function clearData() { if (!confirm('모든 외식 기록과 설정이 삭제됩니다. 이 작업은 되돌릴 수 없습니다.')) return; const { error } = await supabase.from('dining_records').delete().eq('household_id', householdId); if (error) return appError(error); state = { ...seed, records: [] }; await persistSettings(); layout(); }
