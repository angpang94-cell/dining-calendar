...
let activeTab = 'home';
let filter = { start: iso(startOfMonth(today)), end: iso(endOfMonth(today)), label: '이번 달', categories: [...categories] };
let editingId = null;
...
function categoryFilterPanel() { return `<section class="category-filter"><div class="section-heading"><div><h2>항목 선택</h2><p>여러 항목을 선택하면 합산해서 보여드려요.</p></div></div><div class="category-chips">${categories.map(category => `<button type="button" class="chip ${filter.categories.includes(category) ? 'selected' : ''}" data-category-filter="${category}" aria-pressed="${filter.categories.includes(category)}">${category}</button>`).join('')}</div></section>`; }
...
function recordsView() { const rs = recordsBetween(filter.start, filter.end).filter(record => filter.categories.includes(record.category)); return `${topbar('기록', `<button class="icon-button" data-action="add" aria-label="기록 추가">${icon('plus')}</button>`)}<div class="page-content"><div class="page-intro"><p>내가 먹은 맛있는 순간을 모아봤어요.</p></div>${filterPanel()}${categoryFilterPanel()}${rangeSummary(rs)}<section class="section-block"><div class="section-heading"><h2>외식 내역 <span>${rs.length}</span></h2></div>${rs.length ? `<div class="record-list">${rs.map(recordRow).join('')}</div>` : emptyState('선택한 기간과 항목에 외식 기록이 없어요.')}</section></div>${nav()}`; }
